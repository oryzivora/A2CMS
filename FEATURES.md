...
**触发机制**: On-Open Delta Calculation（用户打开应用时按需计算）

由于系统是公网网页,用户不登录使用,关闭网页后下次打开时系统自动计算时间差并应用重置/恢复。
...
```typescript
{
  ...
  energyCap: 840,               // 能量上限
  expeditionCap: 14,           // 远征次数上限
  transcendCap: 7,              // 超越次数上限
  nightmareCap: 14,             // 噩梦次数上限
  shugoCap: 14,                // 树古上限
  shugoMemberExtra: 2,          // 会员树古恢复
  shugoNonMemberExtra: 1,      // 非会员树古恢复
  shopExchangeCap: 20          // 商店/转换奥德上限
}
```
...
| 时间 | 任务 | 恢复量 | 上限 | 维度 |
|------|------|--------|------|------|
| 0:00 | 每日使命 | 重置为上限 | 5 | 账号 |
| 5:00 | 远征次数 | +1 | 14 | 角色 |
| 5:00 | 超越次数 | +1 | 7 | 角色 |
| 5:00 | 噩梦次数 | +2 | 14 | 账号 |
| 5:00 | 树古庆典 | +2(会员)/+1(非会员) | 14 | 账号 |
| 17:00 | 远征次数 | +1 | 14 | 角色 |
| 17:00 | 超越次数 | +1 | 7 | 角色 |
...
| 字段 | 说明 |
|------|------|
| lastVisitTime | 上次访问时间戳 |
| lastEnergyRecoveryTime | 上次能量恢复时间戳 |
| lastDailyResetTime | 上次每日重置时间戳(5:00/17:00) |
| lastDailyMissionResetTime | 上次每日使命重置时间戳(0:00) |
| lastWeeklyResetTime | 上次每周重置时间戳(周三5:00) |
| lastAbyssCorridorResetTime | 上次深渊回廊重置时间戳 |
...
### v0.3.1 (2026-04-26) - Scheduled Tasks Refinements
- **定时任务系统修复**
  - 修复每日副本/每日使命/每周指令未实现重置的问题
  - 修复噩梦次数/树古庆典仅在5:00补充（17:00不再补充）
  - 修复时间戳验证逻辑，防止时钟回拨问题
  - 新增深渊回廊重置功能（周三/周六22:00，国服；21:00，韩服）
  - 删除未使用的 calculateDailyReset 函数

### v0.3.0 (2026-04-25) - Shop/Exchange & UI Refinements
- **定时任务系统** ⭐NEW
  - On-Open Delta Calculation 机制(打开应用时按需计算)
  - 能量恢复: 每3小时,会员+15/非会员+10,上限840
  - 每日重置(5:00/17:00): 远征/超越/噩梦/树古
  - 每周重置(周三5:00): 圣域/深渊指令/商店奥德/转换奥德
  - 韩服时区偏移 -1小时
- **账号系统增强**
